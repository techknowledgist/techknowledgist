...
while read YEAR SOURCE YEAR_FILE; do
	...
	gunzip -c  $infile |  egrep 'ABSTRACT|TITLE' | python $CODEDIR/term_verb.py | sort | uniq -c | sort -nr | python $CODEDIR/reformat_uc2.py > $outpath

    elif [ $SECTIONS == "tas" ]
	then
	gunzip -c  $infile |  egrep 'ABSTRACT|TITLE|SUMMARY' | python $CODEDIR/term_verb.py | sort | uniq -c | sort -nr | python $CODEDIR/reformat_uc2.py > $outpath
