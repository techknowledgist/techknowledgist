...
def np_db_insert_doc_feats(db_path, doc_feats_file):
    ...
    try:
        conn = sqlite3.connect(db_file)
        c = conn.cursor()
        print "[np_db_insert_doc_feats]Opened database in %s" % db_file 
    except:
        e = sys.exc_info()
        print "[np_db_insert_doc_feats]ERROR: Failed to open database in %s, Error: %s" %  (db_file, e) 
    ...
    try:
        ...
        # ignore integrity error.  This is ok - preventing duplicate np/doc_id entries from being
        # inserted.
        pass
    else:
        e = sys.exc_info()
