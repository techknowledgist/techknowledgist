'''
    ...
    Bug fix by Peter Anick 3/13/2015 to handle terms like "mother-in-law"
...
# 3/13/2015 PGA added for debugging singular noun for hyphenated phrases
# >>> ie = inflect.engine()
# >>> ie.singular_noun("continuation-in-part")
import pdb
...
# HANDLE EMPTY WORD, SINGULAR COUNT AND UNINFLECTED PLURALS
        ...
        # PGA: add check for word length to avoid subscript errors on short words later
        if len(lowerword) < 4:
            return word
...
# HANDLE COMPOUNDS ("Governor General", "mother-in-law", "aide-de-camp", ETC.)
        ...
        # modified by PGA 3/13/15.  See comment in quotes below.
        if len(lowersplit) >= 3:
            for numword in range(1, len(lowersplit)-1):
                if lowersplit[numword] in pl_prep_list_da:
                    #pdb.set_trace()
                    ...
                    """
                    PGA 3/13/2015
                    In the original code, for "mother-in-law", we got an error because
                    we are trying to join strings and False.  So I rewrote adding the try/except to test
                    that there are no False components before trying to do the string join.  We either return a 
                    string or False.
                    """
                    try:
                        result = ' '.join(lowersplit[:numword-1] +
                                          [self._sinoun(lowersplit[numword-1], 1, gender=gender) +
                                           '-' + lowersplit[numword] + '-']) + \
                                           ' '.join(lowersplit[(numword+1):])
